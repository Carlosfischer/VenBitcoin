...
<?php 
	...
	if($_POST){
		...
		if ($_POST["clave1"] == $_POST["clave2"] && $_POST["clave1"] =! ""){
			...
			$msgClave = " falta ingresar la clave o que coincida";      
			$claseClave = "error";    
		}
		...
		$host = 'localhost';
			...
			$q = pg_query("SELECT * FROM public.cliente WHERE correo = '$correo'");
			...
			if( $numRe == 0){
				...
				$insert = pg_query("INSERT INTO public.cliente (correo, nombre, apellido, telefono, clave)
			...
			$result = pg_query($cnx,"SELECT * FROM public.cliente");
			if (!$result) {
...
<body>
    ...
    <h1 style="text-align:center;">Registrarse como cliente</h1>
	<form name="fomulariocliente" method="post" action="registrar_usuario.php">
		<fieldset> 
